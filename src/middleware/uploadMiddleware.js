...
export const propertyImageDirectory = path.resolve(uploadDirectory, 'property-images');
mkdirSync(uploadDirectory, { recursive: true });
mkdirSync(propertyImageDirectory, { recursive: true });
...
const propertyImageStorage = multer.diskStorage({
  destination: (_req, _file, callback) => callback(null, propertyImageDirectory),
  filename: (_req, file, callback) => {
    const uniquePrefix = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
    callback(null, `${uniquePrefix}-${safeFileName(file.originalname)}`);
  }
...
const propertyImageFilter = (_req, file, callback) => {
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.mimetype)) {
    return callback(new ApiError(415, 'Upload JPG, PNG, or WEBP images only'));
  }
  return callback(null, true);
};
...
export const propertyImageUpload = multer({
  ...
  limits: { fileSize: Number(process.env.MAX_FILE_SIZE_MB || 10) * 1024 * 1024, files: 10 }
}).array('images', 10);
